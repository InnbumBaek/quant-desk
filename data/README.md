...
러너에서 받아옵니다 — Actions 탭에서 수동 실행(심볼·소스·기간 입력)하거나,
매주 토요일 자동으로 돕니다. 키 없는 소스(stooq → yahoo 순서로 자동 대체)를
기본으로 쓰므로 사전 설정이 필요 없습니다.

로컬에서 직접 받으려면(외부 접속이 되는 환경에서):
...
Claude가 일하는 연구 컨테이너는 시세 호스트 외부 접속이 정책상 차단됩니다
(모든 벤더 호스트가 egress 프록시에서 거부됨). 그래서 수집은 러너에서 합니다.

가격 데이터는 **커밋하지도, 워크플로 아티팩트로 올리지도 않습니다.** 이 리포는
public이고, public 리포의 아티팩트는 누구나 내려받을 수 있어서 재배포에
해당합니다. 무료 EOD 소스 중 재배포를 허용하는 것은 없습니다. 남는 것은
매니페스트(해시·날짜·행수)와 게이트 판정뿐이고, 그것으로 재현이 가능합니다 —
같은 소스에서 같은 기간을 다시 받아 해시를 맞춰보면 됩니다. 근거는 ADR-0007.
